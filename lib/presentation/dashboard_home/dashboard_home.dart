...
class _DashboardHomeState extends State<DashboardHome> {
  ...
  Future<void> _loadMonthlyProfit() async {
    ...
    try {
      ...
      final currentTxs = await _transactionsRepository
      ...
      final prevTxs = await _transactionsRepository.getTransactionsByDateRange(
        prevMonthStart,
        prevMonthEnd,
      );
  ...
  Future<void> _loadTopExpenses() async {
    ...
    try {
      ...
      final expenseTransactions = transactions
          .where((tx) => (tx['type'] as String?) == 'expense')
          .toList();
      ...
      // Create list with percentages
      final List<Map<String, dynamic>> categoriesWithPercentages = [];

      categoryTotals.forEach((categoryId, amount) {
        final percentage =
            totalExpenses > 0 ? (amount / totalExpenses) * 100 : 0.0;
        categoriesWithPercentages.add({
  ...
  Future<void> _loadCashBalanceData() async {
    ...
    try {
      ...
      final transactions = await _transactionsRepository
          .getTransactionsByDateRange(startDateNormalized, endDate);
